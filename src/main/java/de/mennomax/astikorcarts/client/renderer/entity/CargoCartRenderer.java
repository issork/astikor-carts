    ...
    protected void renderContents(final CargoCartEntity entity, final float delta, final MatrixStack stack, final IRenderTypeBuffer source, final int packedLight) {
        ...
        for (int i = 0; i < cargo.size(); i++) {
            ...
            this.attach(this.model.getBody(), this.model.getBody(), s -> {
                ...
                if (model.isGui3d()) {
                    s.translate(x, -0.46D, z);
                    s.scale(0.65F, 0.65F, 0.65F);
                    s.rotate(Vector3f.ZP.rotationDegrees(180.0F));
                    if (iz < 1 && itemStack.getItem().isIn(ItemTags.BEDS)) s.translate(0.0D, 0.0D, 1.0D);
                } else {
                    s.translate(x, -0.15D + ((ix + iz) % 2 == 0 ? 0.0D : 1.0e-4D), z);
                    s.scale(0.7F, 0.7F, 0.7F);
                    s.rotate(Vector3f.YP.rotationDegrees(Objects.hashCode(itemStack.getItem().getRegistryName()) / 65536.0F));
                    s.rotate(Vector3f.XP.rotationDegrees(-90.0F));
                }
                renderer.renderItem(itemStack, ItemCameraTransforms.TransformType.NONE, false, s, source, packedLight, OverlayTexture.NO_OVERLAY, model);
            }, stack);
        }
    }
